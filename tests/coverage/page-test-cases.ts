...
export const pageTestCases: PageTestCase[] = [
  ...
  pageCase({
    ...
    primaryAction:
      "维护个人或 Agent Memory，组合 Skill 与受控 Tool，连接并同步 MCP Server，或一键部署只读系统数据 MCP 与盘点 Agent，审批 MCP Tool，查看熔断/half-open、配额、Worker Job 和结算账本",
    desktop:
      "五个治理 Tab 保持紧凑表格密度；MCP Server、连接和 Tool 策略分层展示，内置系统数据 MCP 明确标记只读且可重复部署，长错误与 Endpoint 不撑破表格；任务、熔断和账本可按状态扫描；可执行任务超过阈值未领取或运行租约过期时显示 Worker 队列异常、数量和最老等待时长，并保持轮询恢复",
    empty:
