...
configRoutes.get("/config/group", authRequired(), ability("system.config.query"), async (c) => {
...
configRoutes.get("/config/items", authRequired(), ability("system.config.query"), async (c) => {
  const page = await buildListQuery(c.req.url, {
