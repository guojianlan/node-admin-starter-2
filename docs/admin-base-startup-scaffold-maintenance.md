...
| P1     | `ADMIN_BASE_ADMIN_PASSWORD` | 待做   | 首次 seed 可配置管理员初始密码             |
| P1     | 生产环境密钥校验            | 待做   | 生产不能使用默认 `ADMIN_BASE_SECRET_KEY`   |
| P1     | `pnpm doctor`               | 待做   | 只检查不修改，输出环境健康状态             |
| P1     | `GET /api/ready`            | 待做   | 判断 DB、默认数据、默认存储是否可用        |
