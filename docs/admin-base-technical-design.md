...
- 上传到 `storage/uploads/YYYYMMDD/<random>.<ext>`。
- `sys_file` 保存原始文件名、路径、大小、扩展名、类型、上传者、分组。
- 提供上传、列表、软删除、恢复、永久删除、下载。
- S3/R2/OSS 放到后续扩展，不阻塞基础框架。
...
不要一开始就做文件管理、主题设置、国际化、AI、邮件配置。基础 admin 框架的关键路径是：

```text
登录 -> 权限 -> 菜单 -> 通用 CRUD -> 用户/角色/菜单管理闭环
```
