...
| GET            | `/api/system/ai/notebook/:id/sources/search/candidates`             | List persisted research candidates and their acceptance/parsing/indexing status                            |
...
| POST           | `/api/system/ai/governance/mcp/internal/oauth/token`                | Exchange the built-in MCP client credentials for signed access and refresh tokens                          |
| POST           | `/api/system/ai/governance/mcp/internal`                            | Serve authenticated read-only System Data MCP JSON-RPC calls                                               |
| POST           | `/api/system/ai/governance/mcp/internal/provision`                  | Provision/update the built-in MCP, allowlist five read-only Tools, and bind the inspection Agent           |
...
| GET            | `/api/system/ai/governance/jobs`                                    | Page PostgreSQL Worker Jobs with stale-queue and expired-lease health                                      |
...
| GET            | `/api/system/ai/chat/sessions/:id/runs/:runId/events`               | Replay persisted Agent Run events after `Last-Event-ID` or `afterEventId`                                  |
...
| GET            | `/api/system/ai/workflow/visual/definitions`                        | List versioned visual Workflow definitions                                                                |
| GET            | `/api/system/ai/workflow/visual/definitions/:id`                    | Read the current visual graph and version                                                                 |
| POST           | `/api/system/ai/workflow/visual/versions`                           | Save a validated visual Workflow draft                                                                     |
| POST           | `/api/system/ai/workflow/visual/definitions/:id/versions/:version/publish` | Publish one immutable visual Workflow version                                                        |
| POST           | `/api/system/ai/workflow/visual/definitions/:id/runs`               | Execute a published visual Workflow                                                                        |
...
| GET            | `/api/system/ai/workflow/runs/:id/events`                           | Stream persisted Workflow progress with LISTEN/NOTIFY wake-up and replay fallback                          |
...
Notebook Web Search is a discovery workflow, not a content source. `sources/search` uses the configured Tavily, Brave, or SearXNG priority/fallback chain and persists sanitized result metadata as `candidate` records; snippets never become Knowledge正文. `sources/search/candidates` exposes the lifecycle for UI inspection. `sources/search/import` deduplicates at most 10 selected URLs and refetches each through the same SSRF-safe Website Source pipeline; candidates move through `accepted`, `parsing`, and `ready` or `failed`, while imported, reused, and failed results are reported independently. Deep Research creates a queued `notebook-deep-research` Workflow Run and Worker Job, persists `plan`, `search`, `import_source`, and `synthesize_report` Steps, round-robins candidates across planned queries, and generates a cited `brief` Artifact only from documents imported by that run. Active runs can be cancelled cooperatively; failed imports remain inspectable without rolling back successful sources.
...
AI Chat stores sessions in `sys_ai_chat_session` and messages in `sys_ai_chat_message`. Sessions are scoped to the current user. Assistant messages transition through `streaming`, `completed`, `stopped`, or `failed`; stale streaming records are recovered as failed. Context governance uses a CJK-aware conservative estimate, reserves output tokens, keeps a recent-message window, and stores a deterministic summary of compacted history. Agent execution persists `sys_ai_agent_run`, `sys_ai_agent_run_step`, and `sys_ai_tool_approval` records. Approval decisions are claimed atomically; duplicates return HTTP 409 and cannot execute a tool twice. An approved tool produces a linked continuation Run (`parentRunId` and `sourceApprovalId`), so refresh/retry can recover the chain without presenting it as an unrelated Run. Approval records can include `planHash`, affected files, validation output, expiry, approver, decision time, and execution time. Expired, denied, or replayed approvals do not execute their tool. Tool execution with a Run lease records `sys_ai_tool_execution` under `runId + attempt + toolCallId`; a completed record returns the saved result, while a concurrent or failed duplicate is rejected closed. Chat SSE persists an event id before delivery, and the client can replay `meta`, `delta`, Tool, source, approval, finish, and error events after a disconnect without submitting the user message again. Server-side tools are selected from one fixed handler registry that owns input schema, risk, approval defaults, and execution dispatch; configuration cannot inject executable code.
...
Mastra Workflows are selected from a server-side static registry for built-in governance checks and
from versioned visual definitions for business orchestration. Visual definitions are stored in
`sys_ai_workflow_definition` and `sys_ai_workflow_definition_version`; saving, publishing and running
requires Admin Base abilities and operation logs. Linear visual graphs are executed by the real Mastra
Workflow runtime and persist `sys_ai_workflow_run`/`sys_ai_workflow_run_step`; non-linear graphs use the
validated Admin Base graph executor until dynamic Mastra graph support is proven. The first static
Workflow, `ai-runtime-preflight`, deterministically checks Agent, Provider, Model, Tool Registry,
approval policy, RequestContext, and data-scope readiness. Mastra runtime DDL remains disabled; all
governance tables are owned by the Admin Base migration chain. Workflow progress is persisted first,
then pushed with PostgreSQL LISTEN/NOTIFY and replayed by event ID when notifications are missed.
